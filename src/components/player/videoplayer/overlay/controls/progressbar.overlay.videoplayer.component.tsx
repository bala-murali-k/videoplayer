...
export function ProgressbarControlOverlay () {
    ...
    useEffect(() => {
        ...
        const updateProgress = () => { 
            setCurrentTime(videoPlayerContext.player.currentTime())
            setDuration(videoPlayerContext.player.duration() || 0)
        }

        updateProgress()
        videoPlayerContext.player.on('loadedmetadata', updateProgress)
        videoPlayerContext.player.on('durationchange', updateProgress)
        videoPlayerContext.player.on('timeupdate', updateProgress)

        return () => {
            videoPlayerContext.player.off('loadedmetadata', updateProgress)
            videoPlayerContext.player.off('durationchange', updateProgress)
            videoPlayerContext.player.off('timeupdate', updateProgress)
        }
...
<div
    ...
    onClick={(event) => {
        if (!videoPlayerContext.player || !progressBarReference.current) return;

        const rect = progressBarReference.current.getBoundingClientRect();
        const clickX = event.clientX - rect.left;
        const percent = clickX / rect.width;
        const seekTime = percent * videoPlayerContext.player.duration();
        videoPlayerContext.player.currentTime(seekTime);
    }}
    onMouseMove={(event) => {
